...
export function ProjectCard({ project }: { project: Project }) {
  return (
    <article className="lux-card group relative flex h-full flex-col overflow-hidden rounded-lg border border-zinc-200 bg-white/72 p-5 shadow-sm transition hover:-translate-y-1 hover:border-zinc-300 hover:shadow-soft dark:border-zinc-800 dark:bg-white/[0.04] dark:hover:border-teal-300/60 dark:hover:shadow-soft-dark">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h3 className="mt-3 text-xl font-semibold tracking-tight text-ink dark:text-zinc-50">
            <Link className="relative z-10" href={`/projects/${project.slug}`}>
      ...
      <p className="relative z-10 mt-2 text-sm font-medium text-zinc-700 dark:text-zinc-200">{project.role}</p>
      <p className="relative z-10 mt-4 flex-1 text-sm leading-6 text-zinc-600 dark:text-zinc-300">{project.summary}</p>
